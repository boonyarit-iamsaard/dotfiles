...
- `Review` → `⚠️ Blocked` if any blocking findings exist — Dev phase must be reopened

If blocking findings exist, also update Dev phase:

- `Dev` → `⚠️ Blocked` with note: `See reviews/review-YYYY-MM-DD.md`
...
Only present if Verdict is ⚠️ Blocked.
